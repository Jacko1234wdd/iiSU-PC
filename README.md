...
- **ROM Directory**, **Emulators**, **Display**, **Advanced** -- everything `config.json` holds, edited here and saved with one Save button at the bottom. The Emulators page's "Test Selected..." button checks where a mapping resolves to (and whether that executable is actually found) without starting the AVD. Advanced has a "Show console windows for the AVD and bridge" checkbox for when the logs alone aren't enough -- turns the hidden-window/log-file behavior above back into real, visible consoles for the next Start (trading away that run's log file, since a process can't sensibly have both).
...
Two moments would otherwise show raw desktop around a window that isn't fullscreen yet -- a cold-booting AVD (its window isn't made fullscreen until partway through starting up) and a game handing off from iiSU to its real PC emulator (between iiSU's window minimizing and the emulator's own window taking over). Both are covered with a fullscreen overlay (`bridge/boot_overlay.py`) for that stretch instead, showing what's actually happening ("Booting iiSU-PC..." / "Waiting on DuckStation...") plus a moving loading bar and a randomly-picked, purely-for-fun line underneath (reticulating splines and the like). Turned off automatically while the debug console checkbox above is on, since it would just hide the windows that's for.
