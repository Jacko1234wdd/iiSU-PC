...
Double-click the **desktop shortcut** Setup.bat created to launch straight into iiSU, or run **`iiSU-PC.bat`** (in this folder) for the control panel — live status, Start/Stop with a running status line (not just a spinner), quick buttons to your ROMs folder and the emulator log, and a button into the ROM directory / PC emulator / hotkey / display settings. `config_editor.py`'s Emulators tab also has a "Test Selected..." button to check where a mapping resolves to (and whether that executable is actually found) without starting the AVD.
